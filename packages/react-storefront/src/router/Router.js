...
export default class Router extends EventEmitter {
  ...
  createEdgeConfiguration({ includePWARoutes = true } = {}) {
    ...
    const processRoute = route => {
      ...
      if (edgeHandler) {
        ...
        if (edgeHandler.hostHeader) {
          const backend = edgeHandler.config().proxy.backend
          if (!backends[backend]) {
            backends[backend] = {}
          }
          backends[backend].host_header = edgeHandler.hostHeader
          backends[backend].overwrite_downstream_host_header = true
        }
  ...
  addBackendResponseRoute(backends, origin, path, maxAgeSeconds) {
    let backend = backends[origin]

    if (!backend) {
      backend = backends[origin] = {
        response_router: []
      }
    }

    if (!backend.response_router) backend.response_router = []

    backend.response_router.push({
